...
import { removeFromCart, updateQuantiy } from '../../redux/features/cart/cartSlice';
...
const CartModel = ({ products, isOpen, onClose }) => {
  ...
  const handleQuantity = (type, id) => {
    ...
    dispatch(updateQuantiy(payload));
              ...
              products.map((item, index) => (
                <div key={index} className='flex flex-col md:flex-row md:items-center md:justify-between shadow-md md:p-5 p-2 mb-4'>
                  <div className="flex items-center">
                    <span className='mr-4 px-1 bg-primary text-white rounded-full mb-2'>0{index + 1}</span>
                    <img src={item.image} alt={item.name} className='w-12 h-12 object-cover mb-3' /> {/* Increased margin-bottom for spacing */}
                    <div className="ml-4"> {/* Added margin-left to space the text */}
                    ...
                    <div className='flex flex-row md:justify-start justify-end items-center mt-2'>
                        ...
                        className='size-6 flex items-center justify-center px-1.5 rounded-full bg-gray-200 text-gray-700 hover:bg-primary hover:
                        text-white ml-8'>-</button>
